...
于是这篇并不那么严谨，且仅为满足本吃瓜群众莫名暴增的好奇心的简单粗暴分析报告就应运而生了。


我们抓取、整理2000年以来的中科院院士增选信息以及相关单位的地址和类型，
从人数、学部、单位、双一流大学、区域、单位类型、年龄等多个角度进行统计、可视化和比较分析。

必须为自己点赞的是，本文所有的数据、代码都完全公布，报告基于rmarkdown和Git的可重复框架。
读者运行rmd文件即可得到同样的报告，欢迎更多吃瓜群众围观、挑错和协作改进！
关于数据集和代码相关更详细的说明，参见文末。

首先，我们先看2000年以来，中科院院士增选的简要数据。下面的表格支持搜索和按列排序。
比如，您可以在搜索框输入某所大学、某个专业来查询相关数据。
...
```{r fig.width=7, fig.height=3.5, out.width='600px'}
...
datatable(countDeptDat3Zero2NA,  options = list(pageLength=10), # filter = 'top',
          colnames = c('单位', Y01_21, '和'), rownames = NULL)
```
...
```{r fig.width=6.5, fig.height=8, out.width='600px'}
tmp2 = emplSum[1:30,]
...
```{r shuangyiliu, fig.width=6, fig.height=8, out.width='600px'}
## 36所双一流A类
...
- 20+个院士入选的高校有`r length(subset(s36, Sum>20)$Empl)`所：`r gsub(',','、',toString(subset(s36, Sum>20)$Empl))`
- 10~19个院士入选的高校有`r length(subset(s36, Sum>=10 & Sum<20)$Empl)`所：`r gsub(',','、',toString(subset(s36, Sum>=10 & Sum<20)$Empl))`
- 5~9个院士入选的高校有`r length(subset(s36, Sum>=5 & Sum<10)$Empl)`所：`r gsub(',','、',toString(subset(s36, Sum>=5 & Sum<10)$Empl))`
- 1~4个院士入选的高校有`r length(subset(s36, Sum>=1 & Sum<5)$Empl)`所：`r gsub(',','、',toString(subset(s36, Sum>=1 & Sum<5)$Empl))`
- 0个院士入选的高校有`r length(subset(s36, Sum<0.5)$Empl)`所：`r gsub(',','、',toString(subset(s36, Sum<0.5)$Empl))`
...
```{r fig.width=6.5, fig.height=8, out.width='600px', warning=FALSE}
...
```{r fig.width=6, fig.height=8, out.width='600px'}
...
而此后，年龄总体小幅度上扬，2021年的平均年龄比2011年增加了5岁。
...
此后院士年龄的分布标准差变小，也就是年龄越来越集中。
可以猜测，院士的年龄，可能也被作为一个重要的考察因素。
...
鉴于本吃瓜群众不了解相关业务知识，因此仅从数据表现上阐述一些非常初步的规律。
...
- 部分单位在多个地方有分布，在填写所在地的时候，一般以总部为主，比如北京，这样会有一点副作用
- 少数院士有两个单位，我们在计算的时候只要单位都在国内，都分别都算1个
